...
already exist. If it does not, use `new-case` first. Static intake needs no completed
live-target scope, but never infer authorization to contact anything named inside the
artifact.
...
2. Read the case's `SCOPE.md` and `NOTES.md`. Confirm the artifact belongs to that case.
   If the mapping is ambiguous, ask before copying.
3. Compute SHA-256 at the source:
...
4. Copy, never move, into `cases/<case>/artifacts/`, preserving the basename. If that
   name exists, compare hashes and do not overwrite: identical means intake is already
   complete, different means stop and ask for a distinct filename.
...
7. Put derived files only under `extracted/`. The copy in `artifacts/` is immutable.
